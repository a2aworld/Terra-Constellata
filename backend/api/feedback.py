...
class FeedbackSubmission(BaseModel):
    ...
    feedback_type: str = Field(..., description="Type of feedback (rating, comment, bug_report, feature_request)")
...
async def submit_feedback(feedback: FeedbackSubmission, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        app_logger.info(f"Feedback submitted: {feedback_id}", extra={
            "feedback_id": feedback_id,
            "type": feedback.feedback_type,
            "rating": feedback.rating
        })
...
async def submit_workflow_feedback(workflow_id: str, feedback: WorkflowFeedback, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        app_logger.info(f"Workflow feedback submitted: {feedback_id}", extra={
            "feedback_id": feedback_id,
            "workflow_id": workflow_id,
            "stage": feedback.stage,
            "rating": feedback.rating
        ...
        raise HTTPException(status_code=500, detail="Failed to submit workflow feedback")

@router.post("/artwork/{artwork_id}", response_model=Dict[str, Any])
async def submit_artwork_feedback(artwork_id: str, feedback: ArtworkFeedback, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Store feedback asynchronously
        background_tasks.add_task(
            store_artwork_feedback,
            artwork_id,
            feedback_data
        )

        app_logger.info(f"Artwork feedback submitted: {feedback_id}", extra={
            "feedback_id": feedback_id,
            "artwork_id": artwork_id,
            "rating": feedback.rating
        })
...
async def get_recent_feedback(limit: int = 10):
    ...
    try:
        ...
        recent_feedback = sorted(all_feedback, key=lambda x: x.timestamp, reverse=True)[:limit]

        return {
            "status": "success",
            "data": [f.to_dict() for f in recent_feedback]
        }
    except Exception as e:
...
def store_general_feedback(feedback_data: dict):
    ...
    try:
        ...
        user_feedback = UserFeedback(
            workflow_id=feedback_data.get('workflow_id', f"general_{feedback_data['id']}"),
...
def store_workflow_feedback(workflow_id: str, feedback_data: dict):
    ...
    try:
        user_feedback = UserFeedback(workflow_id, feedback_data.get('user_id', 'anonymous'))
...
def store_artwork_feedback(artwork_id: str, feedback_data: dict):
    ...
    try:
        ...
        if feedback_data.get('content_feedback'):
            user_feedback.add_feedback_text(f"Content: {feedback_data['content_feedback']}")
