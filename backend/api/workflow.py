...
class WorkflowStartRequest(BaseModel):
    ...
    trigger_source: str = Field(..., description="Source of trigger ('human' or 'autonomous')")
...
class FeedbackRequest(BaseModel):
    ...
    curation_decisions: Optional[Dict[str, Any]] = Field(None, description="Curation decisions")
...
async def start_cocreation_workflow(
    ...
    if not cocreation_workflow:
        raise HTTPException(status_code=503, detail="Co-creation workflow not available")
    ...
    try:
        ...
        workflow_id = request.workflow_id or f"cocreation_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
...
async def trigger_autonomous_workflow():
    ...
    try:
        ...
        autonomous_input = "Autonomous exploration of creative territories in geomythology"

        # Start autonomous workflow
        workflow_result = await cocreation_workflow.start_cocreation_workflow(
            trigger_source="autonomous",
            human_input=autonomous_input
        )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to trigger autonomous workflow: {str(e)}")
...
async def get_workflow_statistics():
    ...
    try:
        ...
        return {
            ...
            "average_stages_completed": sum(len(w["stages_completed"]) for w in history) / total_workflows if total_workflows > 0 else 0,
            "timestamp": datetime.utcnow()
        }
