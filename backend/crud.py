...
# Multimedia CRUD
def create_multimedia(db: Session, multimedia: schemas.MultimediaCreate, file_path: str):
...
def get_multimedia(db: Session, multimedia_id: int):
    return db.query(models.Multimedia).filter(models.Multimedia.id == multimedia_id).first()
...
def get_high_cat_score_contents(db: Session, threshold: float = 0.8):
    return db.query(models.Content).filter(models.Content.cat_score >= threshold, models.Content.is_published == False).all()
