...
class PuzzlePiecesIngestion:
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "row_number" in cleaned_df.columns:
            cleaned_df["row_number"] = pd.to_numeric(
                cleaned_df["row_number"], errors="coerce"
            ).astype("Int64")

        # Clean coordinates
        for coord in ["latitude", "longitude"]:
            if coord in cleaned_df.columns:
                cleaned_df[coord] = pd.to_numeric(cleaned_df[coord], errors="coerce")
    ...
    def validate_data_quality(
        ...
        return (
            len([issue for issue in quality_issues if issue["severity"] == "ERROR"])
            == 0,
            quality_issues,
        )
        ...
        for i in range(0, len(df), batch_size):
            ...
            try:
                ...
                if batch_data:
                    ...
                    logger.info(
                        f"Inserted batch {i//batch_size + 1}: {len(batch_data)} records"
                    )
    ...
    def log_processing_result(
        ...
        try:
            ...
            self.db.execute_command(
                query, (operation, status, records_processed, error_message)
            )
        except Exception as e:
    ...
    def ingest_csv_file(self, file_path: str, batch_size: int = 1000) -> Dict[str, Any]:
        ...
        try:
            ...
            # Insert data
            records_inserted, insert_errors = self.insert_data_batch(
                cleaned_df, batch_size
            )
            ...
            status = (
                "SUCCESS"
                if result["success"]
                else ("PARTIAL" if records_inserted > 0 else "FAILED")
            )
            ...
            logger.info(
                f"Ingestion completed: {records_inserted}/{len(cleaned_df)} records inserted"
            )
            ...
            # Log failure
            self.log_processing_result(
                f"CSV Ingestion: {Path(file_path).name}", "FAILED", 0, error_msg
            )

        return result


def ingest_puzzle_pieces_csv(
    file_path: str, db_config: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
