...
def create_sample_csv(file_path: str):
    ...
    sample_data = [
        ...
        [1, 'New York City', 'city', 'metropolis', 'Major metropolitan area in the US', 'https://example.com/nyc', 40.7128, -74.0060],
        [2, 'Central Park', 'park', 'urban_park', 'Large urban park in Manhattan', 'https://example.com/centralpark', 40.7829, -73.9654],
        [3, 'Eiffel Tower', 'landmark', 'tower', 'Iconic iron tower in Paris', 'https://example.com/eiffel', 48.8584, 2.2945],
        [4, 'Amazon Rainforest', 'natural_feature', 'forest', 'Largest rainforest on Earth', 'https://example.com/amazon', -3.4653, -62.2159],
        [5, 'Mount Everest', 'mountain', 'peak', 'Highest mountain in the world', 'https://example.com/everest', 27.9881, 86.9250],
        [6, 'Sydney Opera House', 'landmark', 'cultural', 'Iconic performing arts venue', 'https://example.com/opera', -33.8568, 151.2153],
        [7, 'Great Wall of China', 'landmark', 'wall', 'Ancient defensive wall in China', 'https://example.com/greatwall', 40.4319, 116.5704],
        [8, 'Sahara Desert', 'natural_feature', 'desert', 'World\'s largest hot desert', 'https://example.com/sahara', 23.4162, 25.6628],
        [9, 'Tokyo Tower', 'landmark', 'tower', 'Communications and observation tower', 'https://example.com/tokyotower', 35.6586, 139.7454],
        [10, 'Grand Canyon', 'natural_feature', 'canyon', 'Steep-sided canyon in Arizona', 'https://example.com/grandcanyon', 36.1069, -112.1129]
    ]
...
def test_pipeline():
    ...
    try:
        ...
        if not init_success:
            logger.warning("Pipeline initialization failed (likely PostgreSQL not running)")
        ...
        # Bounding box query
        bbox_results = pipeline.perform_query('bbox',
        ...
        # Nearby points query
        nearby_results = pipeline.perform_query('nearby',
                                              lon=-74.0, lat=40.7,
...
def test_data_processing():
    ...
    try:
        ...
        test_data = {
            ...
            'source_url': ['https://example.com/1', 'https://example.com/2', 'invalid-url']
        }
        ...
        logger.info(f"Quality score: {quality_report['summary']['overall_quality_score']:.2f}%")
