...
class LearningMetrics:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        avg_improvement = (sum(p['improvement'] for p in self.performance_improvements) /
                          len(self.performance_improvements)) if self.performance_improvements else 0.0

        avg_efficiency = (sum(self.learning_efficiency_history) /
                         len(self.learning_efficiency_history)) if self.learning_efficiency_history else 0.0
        ...
        return {
            ...
            'optimization_success_rate': (self.successful_optimizations /
                                        self.learning_cycles_completed) if self.learning_cycles_completed > 0 else 0.0
        }
...
class CollectiveLearningLoop:
    ...
    def __init__(self, orchestrator: SentinelOrchestrator,
        ...
        self.workflow_tracer = WorkflowTracer(self.config.get('trace_storage_path', './traces'))
        self.pattern_analyzer = PatternAnalyzer(self.workflow_tracer)
        self.feedback_collector = FeedbackCollector(self.config.get('feedback_storage_path', './feedback'))
        self.prompt_optimizer = PromptOptimizer(self.feedback_collector, self.pattern_analyzer)
        self.reward_trainer = RewardModelTrainer(self.config.get('model_storage_path', './models'))
    ...
    async def _learning_cycle(self):
        ...
        while self.is_learning:
            try:
                ...
                logger.info(f"Learning cycle {self.learning_cycle_count} completed in {cycle_duration:.2f}s")
    ...
    async def _execute_learning_cycle(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if new_traces:
                ...
                cycle_results['learning_efficiency'] = self._calculate_learning_efficiency(cycle_results)
    ...
    def _update_pattern_analysis(self, traces: List[WorkflowTrace]) -> int:
        ...
        try:
            ...
            patterns_found = (len(results.get('workflow_patterns', {})) +
            ...
            for trace in traces:
                ...
                if feedback:
                    user_feedback[trace.workflow_id] = {
                        'cat_score': feedback.cat_score.score if feedback.cat_score else 0,
        ...
        try:
            ...
            if self.config.get('enable_prompt_optimization', True):
                for workflow_id in self.feedback_collector.get_high_scoring_workflows():
                    optimization = self.prompt_optimizer.optimize_prompt_from_feedback(workflow_id)
                    if optimization:
            ...
            if recommendations.get('suggested_workflow_patterns'):
                ...
                optimizations_applied += len(recommendations['suggested_workflow_patterns'])
    ...
    async def optimize_orchestrator_decision(self, current_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            pattern_recs = self.pattern_analyzer.get_pattern_recommendations(current_state)
            recommendations['suggested_actions'].extend(pattern_recs.get('suggested_workflow_patterns', []))

            # Get reward model predictions
            if self.reward_trainer.current_model:
                # Predict rewards for different actions
                action_rewards = {}
                for action in ['scan_territories', 'dispatch_agent', 'coordinate_workflow', 'monitor_system']:
            ...
            if current_state.get('task_type'):
                best_template = self.prompt_optimizer.get_best_template(current_state['task_type'])
                if best_template:
                    recommendations['suggested_actions'].append({
    ...
    def _extract_decision_features(self, state: Dict[str, Any], action: str) -> Dict[str, Any]:
