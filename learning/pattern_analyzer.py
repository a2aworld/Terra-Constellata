...
class WorkflowPattern:
    ...
    def add_occurrence(self, trace: WorkflowTrace):
        ...
        if trace.success_metrics:
            self.avg_efficiency = ((self.avg_efficiency * (self.frequency - 1)) +
                                 trace.success_metrics.get('workflow_efficiency', 0)) / self.frequency
            self.avg_duration = ((self.avg_duration * (self.frequency - 1)) +
                               (trace.duration or 0)) / self.frequency

        # Update success rate
        successful_traces = sum(1 for tid in self.supporting_traces
                              if WorkflowTracer().get_trace(tid) and
                              WorkflowTracer().get_trace(tid).error_info is None)
...
class AgentCombinationPattern:
    ...
    def add_combination(self, agents: Tuple[str, ...], performance_score: float,
        ...
        self.agent_combinations[agents] = {
            ...
            'frequency': self.agent_combinations.get(agents, {}).get('frequency', 0) + 1
        }

        # Update best combination
        combined_score = (performance_score * 0.4 + success_rate * 0.4 +
                         (1.0 / max(avg_duration, 0.1)) * 0.2)  # Efficiency bonus
    ...
    def get_optimal_combination(self, max_agents: int = 3) -> Tuple[str, ...]:
        ...
        candidates = [(agents, metrics) for agents, metrics in self.agent_combinations.items()
        ...
        candidates.sort(key=lambda x: (x[1]['performance_score'] * 0.4 +
...
class DecisionPattern:
    ...
    def get_best_action(self, condition: str) -> Optional[str]:
        ...
        for action in actions_data['actions']:
            success_rate = actions_data['successes'][action] / actions_data['actions'][action]
    ...
    def get_action_probabilities(self, condition: str) -> Dict[str, float]:
        ...
        return {action: count / total_actions
...
class PatternAnalyzer:
    ...
    def analyze_patterns(self, workflow_type: Optional[str] = None,
        ...
        pattern_quality = self._calculate_pattern_quality(workflow_patterns, agent_patterns)
        ...
        logger.info(f"Pattern analysis completed. Found {len(workflow_patterns)} workflow patterns, "
                   f"{len(agent_patterns)} agent patterns, {len(decision_patterns)} decision patterns")

        return results

    def _analyze_workflow_patterns(self, traces: List[WorkflowTrace],
                                 min_frequency: int) -> Dict[str, WorkflowPattern]:
    ...
    def _analyze_agent_patterns(self, traces: List[WorkflowTrace]) -> Dict[str, AgentCombinationPattern]:
        ...
        for trace in traces:
            ...
            agent_names = tuple(sorted(set(interaction['agent_name']
            ...
            task_type = trace.agent_interactions[0]['agent_name'].split('_')[0] if '_' in trace.agent_interactions[0]['agent_name'] else 'general'
    ...
    def _analyze_decision_patterns(self, traces: List[WorkflowTrace]) -> Dict[str, DecisionPattern]:
            ...
            for edge in trace.edges_traversed:
                if edge.get('condition'):
                    ...
                    if decision_point not in decision_patterns:
                        decision_patterns[decision_point] = DecisionPattern(decision_point)
    ...
    def _calculate_pattern_quality(self, workflow_patterns: Dict[str, WorkflowPattern],
                                 agent_patterns: Dict[str, AgentCombinationPattern]) -> Dict[str, Any]:
        ...
        if workflow_patterns:
            avg_success_rate = np.mean([p.success_rate for p in workflow_patterns.values()])
            avg_frequency = np.mean([p.frequency for p in workflow_patterns.values()])
            avg_efficiency = np.mean([p.avg_efficiency for p in workflow_patterns.values()])

            workflow_quality = (avg_success_rate * 0.4 + avg_efficiency * 0.4 +
                              min(avg_frequency / 10, 1.0) * 0.2)

        # Calculate agent pattern quality
        agent_quality = 0.0
        if agent_patterns:
            best_scores = [p.best_score for p in agent_patterns.values() if p.best_score > 0]
            if best_scores:
    ...
    def get_optimal_workflow_pattern(self, workflow_type: str) -> Optional[WorkflowPattern]:
        """Get the optimal workflow pattern for a given type."""
        patterns = [p for p in self.workflow_patterns.values()
        ...
        return max(patterns, key=lambda p: p.success_rate * 0.5 + p.avg_efficiency * 0.3 + min(p.frequency / 10, 1.0) * 0.2)

    def get_optimal_agent_combination(self, task_type: str, max_agents: int = 3) -> Tuple[str, ...]:
    ...
    def predict_workflow_success(self, workflow_pattern: List[str],
                               agent_combination: Tuple[str, ...]) -> float:
        ...
        if agent_combination and best_pattern.agent_sequence:
            agent_overlap = len(set(agent_combination) & set(best_pattern.agent_sequence))
            agent_bonus = agent_overlap / max(len(agent_combination), len(best_pattern.agent_sequence))
        ...
        for decision_point, pattern in self.decision_patterns.items():
            ...
            if best_action:
                recommendations['decision_recommendations'][decision_point] = {
                    ...
                    'success_rate': pattern.action_success_rates.get(f"{condition}:{best_action}", 0.0)
                }
